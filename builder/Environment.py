...
class Environment:
    def __init__(self, options, targetsDict):
        self.source_dir = os.path.join(options.working_dir, 'SOURCE')
        self.build_dir = os.path.join(options.working_dir, "BUILD_{}".format(options.target_platform))
        self.archive_dir = os.path.join(options.working_dir, "ARCHIVE")
        self.toolchain_dir = os.path.join(options.working_dir, "TOOLCHAINS")
        self.log_dir = os.path.join(self.build_dir, 'LOGS')
        self.install_dir = os.path.join(self.build_dir, "INSTALL")

        dirs = [self.source_dir, self.build_dir, self.archive_dir, self.toolchain_dir, self.log_dir, self.install_dir]
        for dir in dirs:
    ...
    def detect_distname() -> str:
        ...
        if distname == 'Windows':
            ...
            print('WARNING: kiwix-build has not been tested on macOS platform.\n'
                  'Tests, bug reports and patches are welcomed.')
        elif distname == 'Linux':
            # TODO: linux_distribution is deprecated in python 3.5
    ...
    @staticmethod
    def detect_meson() -> str:
        meson = None
        for name in ['meson.py', 'meson']:
            try:
                subprocess.check_call([name, '--version'], stdout=subprocess.DEVNULL)
                meson = name
                break
